...
const HealthTips = () => {
  ...
  return (
    <div className="py-6 px-3 sm:py-12 sm:px-6 lg:px-8 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen pb-24">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white p-3 sm:p-4 rounded-lg shadow-md mb-4 sm:mb-6">
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2 sm:px-4 sm:py-3 pl-10 pr-10 sm:pr-12 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm sm:text-base"
              />
          ...
          <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2">
            {categories.map((category) => (
              <button
                ...
                className={`px-2.5 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300 ${
        ...
        {/* Floating Tips Grid - 2 columns on mobile, 3 on tablet, 4 on desktop */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
          {currentTips.map((tip) => (
            ...
            >
              <div className="bg-white rounded-lg sm:rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 h-full flex flex-col">
                <div className={`${tip.color} p-2 sm:p-4 flex items-center justify-between`}>
                  <div className="flex items-center space-x-1 sm:space-x-2">
                    <div className="p-1 sm:p-2 rounded-full bg-white bg-opacity-30">
                      <div className="w-4 h-4 sm:w-6 sm:h-6 text-white">
                        {getIcon(tip.icon)}
                      </div>
                    </div>
                    <span className="text-white font-medium text-xs sm:text-sm truncate">{tip.category}</span>
                  </div>
                  <div className="animate-pulse">
                    <Sparkles className="w-3 h-3 sm:w-5 sm:h-5 text-white opacity-70" />
                  </div>
                </div>
                <div className="p-3 sm:p-6 flex-grow">
                  <h3 className="text-sm sm:text-xl font-bold text-gray-900 mb-1 sm:mb-2 line-clamp-2">{tip.title}</h3>
                  <p className="text-xs sm:text-base text-gray-600 line-clamp-3">{tip.description}</p>
                </div>
                <div className="px-3 pb-3 sm:px-6 sm:pb-6">
                  <button 
                    onClick={() => window.open(tip.link, "_blank")}
                    className="flex items-center text-blue-600 hover:text-blue-800 transition-colors duration-300 group text-xs sm:text-base"
                  >
          ...
          <div className="mt-6 sm:mt-12 flex justify-center">
            <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
              <button
                ...
                className={`relative inline-flex items-center px-2 py-1.5 sm:px-2 sm:py-2 rounded-l-md border border-gray-300 bg-white text-xs sm:text-sm font-medium ${
              ...
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`relative inline-flex items-center px-2.5 py-1.5 sm:px-4 sm:py-2 border text-xs sm:text-sm font-medium ${
                    currentPage === i + 1
              ...
              <button
                ...
                className={`relative inline-flex items-center px-2 py-1.5 sm:px-2 sm:py-2 rounded-r-md border border-gray-300 bg-white text-xs sm:text-sm font-medium ${
